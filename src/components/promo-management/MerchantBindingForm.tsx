...
export default function MerchantBindingForm({ promoId, onSuccess }: Props) {
  ...
  const fetchMerchants = async (keyword: string) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/api/promo/merchant-search?keyword=${encodeURIComponent(keyword)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/promo/merchant-bind/${promoId}`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Button type="submit" className="w-full" disabled={isSubmitting}>
        {isSubmitting ? "Binding..." : "Bind Merchant"}
      </Button>
    </form>
  );
}
