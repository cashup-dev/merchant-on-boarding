...
export default function TableUpload({
  ...
  const resolvedInitialFiles = initialFiles ?? (showDefaults ? defaultFiles : []);

  // Convert default files to FileUploadItem format
  const defaultUploadFiles: FileUploadItem[] = resolvedInitialFiles.map((file) => ({
    id: file.id,
    file: {
      name: file.name,
      size: file.size,
  ...
  const [queuedFiles, setQueuedFiles] = useState<FileWithPreview[] | null>(null);
  ...
  useEffect(() => {
    if (!queuedFiles) return;
    onFilesChange?.(queuedFiles);
    setQueuedFiles(null);
  }, [queuedFiles, onFilesChange]);
